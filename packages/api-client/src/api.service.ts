...
import { GuildQuery, HypixelCache, LeaderboardQuery } from "./constants.js";
...
export class ApiService {
	...
	public getRecentGames(tag: string) {
		return this.requestKey<GetRecentGamesResponse, "recentGames">("/player/recentgames", "recentGames", {
			player: tag,
		});
	}
	...
	public getPlayerLeaderboard(field: string, input: string | number, type: LeaderboardQuery): Promise<PostLeaderboardResponse | null> {
	...
	public getPlayerRankings(fields: string[], uuid: string) {
		return this.request<PostLeaderboardRankingsResponse[]>("/player/leaderboards/rankings", {}, "POST", { body: { fields, uuid } });
	}
	...
	public getGuildLeaderboard(field: string, input: string | number, type: LeaderboardQuery): Promise<PostLeaderboardResponse | null> {
	...
	public resetPlayerSession(tag: string) {
		return this.request<GetPlayerResponse>("/session", { player: tag }, "DELETE");
	}
	...
	public verifyUser(codeOrUuid: string, id: string) {
		return this.request<GetUserResponse>("/user", { [codeOrUuid.length >= 32 ? "uuid" : "code"]: codeOrUuid, id }, "PUT")
			.then((data) => data.user ?? null)
			.catch(() => null);
	}
	...
	private async requestKey<T, K extends keyof T>(url: string, key: K, params?: Record<string, unknown>, method: Method = "GET") {
