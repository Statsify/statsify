...
export const config = <T extends keyof FlatConfig>(key: T, { required = true, default: defaultValue }: ConfigOptions<T> = {}): FlatConfig[T] => {
	...
	if (!isValueDefined) {
		...
		if (required) throw new Error(`Missing required environment variable: ${key as string} | Add ${key as string} to your config`);
	}
