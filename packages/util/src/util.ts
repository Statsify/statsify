...
export const findScoreIndex = <T extends { req: number }>(data: T[], score = 0): number =>
	data.findIndex(({ req }, index, arr) => score >= req && ((arr[index + 1] && score < arr[index + 1].req) || !arr[index + 1]));
...
export const arrayGroup = <T extends Array<any> | string>(arr: T, groupSize: number): T[] =>
	Array.from({ length: Math.ceil(arr.length / groupSize) }, (_, i) => arr.slice(i * groupSize, (i + 1) * groupSize)) as T[];
...
if (import.meta.vitest) {
	...
	test("romanNumeral", () => {
		it("should give the correct roman numeral", () => {
			...
			expect(romanNumeral(99_999)).toBe("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMCMXCIX");
		});
	});
	...
	test("formatTime", () => {
		it("should format time", () => {
			...
			expect(formatTime(day + hour + minute + second, { short: false, entries: 4 })).toBe("1 day, 1 hour, 1 minute, 1 second");
			expect(formatTime((day + hour + minute + second) * 2, { short: false, entries: 4 })).toBe("2 days, 2 hours, 2 minutes, 2 seconds");
			...
			expect(formatTime((day + hour + minute + second) * 2, { short: true, entries: 4 })).toBe("2d, 2h, 2m, 2s");
			expect(formatTime((day + hour + minute + second) * 2, { short: true, entries: 2 })).toBe("2d, 2h");
		});
	});
}
