...
const normalizeNode = (node: ElementNode, side: Side, other: Side) => {
	...
	if (sideType === "undefined" || (sideType === "number" && (node[side].size as number) < node[side].minSize)) node[side].size = node[side].minSize;
...
const processSideData = (data: SideData) => {
	...
	return (
		(data.highestPixelToPercentRatio > remainingPixelPercentRatio ? data.highestPixelToPercentRatio : remainingPixelPercentRatio) + data.paddedLength
	);
};

export const elementToNode = (type: IntrinsicElement, { dimension, style, children, props }: RawElement): ElementNode => {
