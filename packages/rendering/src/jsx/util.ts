...
import type { ElementNodeBiDirectional, Fraction, Instruction, InstructionBiDirectional, Percent } from "./types.js";

export const toDecimal = (measurement: Percent | Fraction): number => {
	if (measurement.endsWith("%")) return Number.parseFloat(measurement.replace("%", "")) / 100;
...
const getMargin = (bidirectional: InstructionBiDirectional | ElementNodeBiDirectional) => bidirectional.margin1 + bidirectional.margin2;

const getPadding = (bidirectional: InstructionBiDirectional | ElementNodeBiDirectional) => bidirectional.padding1 + bidirectional.padding2;
...
export const getTotalSize = (
	...
	if (size) {
		s += typeof bidirectional.size === "number" ? bidirectional.size : bidirectional.minSize;
	}
...
const getChildrenSize = (instruction: Instruction, side: "x" | "y", { margin = true, padding = true, size = true }: GetTotalSizeOptions = {}) => {
