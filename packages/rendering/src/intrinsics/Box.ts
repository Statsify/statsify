...
function increaseSpacing(spacing: JSX.Spacing, side: keyof JSX.CompleteSpacing, amount: number) {
...
export const renderSnow = (ctx: CanvasRenderingContext2D, winterTheme: WinterThemeService, x: number, y: number, width: number) => {
	...
	while (drawnSnow < snowWidth) {
		...
		ctx.drawImage(centerSnow, 0, 0, drawn, centerSnow.height, x + drawnSnow + leftSnow.width, y - SNOW_OFFSET, drawn, centerSnow.height);
...
export const render: JSX.Render<BoxRenderProps> = (
	...
	if (border.bottomRight !== 0) ctx.fillRect(x + width - border.bottomRight, y + height - border.bottomRight, border.bottomRight, border.bottomRight);
