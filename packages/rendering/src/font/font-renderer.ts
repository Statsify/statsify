...
export class FontRenderer {
	...
	public fillText(ctx: CanvasRenderingContext2D, nodes: TextNode[], x: number, y: number) {
		...
		for (const { text, color, bold, italic, underline, strikethrough, size } of nodes) {
			...
			for (const char of text) {
				x += this.fillCharacter(ctx, char, Math.round(x), Math.round(adjustY), size, bold, italic, underline, strikethrough, color);
			}
		}
	}
	...
	private getCharacterSpacing(char: string, isAscii: boolean, size: number, scale: number, width: number, bold: boolean) {
	...
	private fillCharacter(
		...
		this.fillFormattedCharacter(ctx, imageData, x, y, width, scale, size, resizeFactor, color, underline, strikethrough, bold, italic, isAscii);
	...
	private fillFormattedCharacter(
		...
		if (bold) {
			this.fillPlainCharacter(ctx, imageData, x + resizeFactor, y, width, scale, size, color, italic);

			const offset = x + 2 * resizeFactor;

			if (isAscii) this.fillPlainCharacter(ctx, imageData, offset, y, width, scale, size, color, italic);
	...
	private fillLine(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, resizeFactor: number, color: Fill) {
		ctx.fillStyle = color;

		ctx.fillRect(x - 2 * resizeFactor, y, (width + 2) * (resizeFactor * 2), resizeFactor * 2);
	}

	private fillUnderline(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, resizeFactor: number, color: Fill) {
		return this.fillLine(ctx, x, y + 16 * resizeFactor, width, resizeFactor, color);
	}

	private fillStrikethrough(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, resizeFactor: number, color: Fill) {
