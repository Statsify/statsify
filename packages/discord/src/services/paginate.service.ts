...
export async function paginate(
  ...
  const getMessage = async (index: number, subIndex: number) => {
    ...
    const message = await contentToMessage(content);
  ...
  let subController =
    "subPages" in page && page.subPages.length > 1
      ? new PageController(page.subPages, currentSubIndex)
      : undefined;

  mainController.register(listener, (interaction, index) =>
    handler(interaction, index, 0),
  );
  subController?.register(listener, (interaction, subIndex) =>
    handler(interaction, currentIndex, subIndex),
  );
  ...
  async function handler(
    ...
    message.components = [
      mainController.getActionRow(),
      subController?.getActionRow(),
 ...
 * @param timeout When to stop the pagination (ms), defaults to 300000
 */
export async function scrollingPagination(
  context: CommandContext,
  ...
  for (const [index, component] of controller.entries()) {
    listener.addHook(component.getCustomId(), async (interaction) => {
      ...
      if (index === 0) {
        // Backwards
        page = currentIndex === 0 ? pages.length - 1 : currentIndex - 1;
      } else {
        // Forwards
        page = currentIndex === pages.length - 1 ? 0 : currentIndex + 1;
      }
  ...
  function onTimeout() {
    for (const component of controller) {
      listener.removeHook(component.getCustomId());
    }
  ...
  const message = await getMessage(currentIndex);
  message.components = [new ActionRowBuilder(controller)];

  return message;
}

async function contentToMessage(
  content: PaginateInteractionContent,
): Promise<Message> {
  if (content instanceof Message) return content;
  if (content instanceof EmbedBuilder)
    return new Message({ embeds: [content] });
  if (content instanceof Canvas)
    return new Message({
      files: [
        {
          name: "image.png",
          data: await content.toBuffer("png"),
          type: "image/png",
        },
      ],
      attachments: [],
...
class PageController {
  ...
  public switchPage(index: number) {
    ...
    for (const [i, button] of this.#menu.entries()) {
      button.style(i === index ? ButtonStyle.Primary : ButtonStyle.Secondary);
    }
  }
  ...
  public register(
    ...
    for (const [index, button] of this.#menu.entries()) {
      listener.addHook(button.getCustomId(), (interaction) =>
        handler(interaction, index),
      );
    }
  }
  ...
  public unregister(listener: AbstractCommandListener) {
    if (this.#menu instanceof SelectMenuBuilder) {
      listener.removeHook(this.#menu.getCustomId());
    } else {
      for (const button of this.#menu) {
