...
import { APIUser, ApplicationCommandOptionType, GatewayDispatchEvents, InteractionResponseType } from "discord-api-types/v10";
...
import type { Interaction as DiscordInteraction, InteractionResponse, InteractionServer, RestClient, WebsocketShard } from "tiny-discord";

const COMMAND_INTERACTION_RESPONSE = {};
type CommandInteractionResponse = typeof COMMAND_INTERACTION_RESPONSE;

export type InteractionHook = (interaction: Interaction) => InteractionResponse | void | Promise<any>;
...
export abstract class AbstractCommandListener {
	...
	protected constructor(client: InteractionServer, rest: RestClient, commands: Map<string, CommandResolvable>, applicationId: string, port: number);
	protected constructor(client: WebsocketShard, rest: RestClient, commands: Map<string, CommandResolvable>, applicationId: string);
	...
	protected getCommandAndData(command: CommandResolvable, data: any, name = command.name): [command: CommandResolvable, data: any, name: string] {
	...
	protected async executeCommand({ commandName, command, context, preconditions = [], message }: ExecuteCommandOptions) {
	...
	private async onInteraction(interaction: Interaction): Promise<InteractionResponse | CommandInteractionResponse> {
	...
	private handleWebsocketShard(client: WebsocketShard) {
		client.on("ready", (data) => {
			this.logger.log(`Connected to gateway with WebsocketShard on ${(data.user as APIUser).username}`);
		});

		client.on("event", async (event) => {
			if (event.t !== GatewayDispatchEvents.InteractionCreate) return;

			const interaction = new Interaction(this.rest, event.d as DiscordInteraction, this.applicationId);
