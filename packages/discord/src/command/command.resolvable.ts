...
export class CommandResolvable {
	public type: ApplicationCommandType.ChatInput | ApplicationCommandOptionType.Subcommand | ApplicationCommandOptionType.SubcommandGroup;
	public name: string;
	...
	public constructor({ name, description, args, methodName, tier = UserTier.NONE, preview, cooldown = 10 }: CommandMetadata, target: any) {
		this.name = name;
	...
	public equals(other: CommandResolvable): boolean {
		...
		if (d.description_localizations && other.description_localizations) {
			for (const key in this.description_localizations) if (d.description_localizations[key] !== other.description_localizations[key]) return false;
		}
