...
import { getBounty, getLevel, getLevelFormatted, getPrestige, getPrestigeReq } from "./util.js";

export const PIT_MODES = new GameModes([{ api: "overall", hypixel: "PIT", formatted: "Pit" }]);
...
export class Pit {
	...
	public constructor(profile: APIData, data: APIData) {
		...
		this.progression = new Progression(this.exp - lastPrestigeReq, Math.min(getPrestigeReq(prestige) - lastPrestigeReq, 11_787_293_080));
		...
		this.totalMysticsEnchanted = add(this.tier1MysticsEnchanted, this.tier2MysticsEnchanted, this.tier3MysticsEnchanted);
