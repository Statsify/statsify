...
export class BridgeDuels {
	...
	public "constructor"(data: APIData) {
		...
		this.overall = deepAdd(this.solo, this.doubles, this.threes, this.fours, this["2v2v2v2"], this["3v3v3v3"], this.ctf);
		...
		const { formatted, bold, semi, max, index, color, req, inc } = getTitle(this.overall.wins, "Bridge");

		this.titleFormatted = formatted;

		this.titleLevelFormatted = `${color.code}${bold || semi ? "§l" : ""}[${romanNumeral(index)}]`;

		const nextData = getTitle(req + index * inc, "Bridge");

		this.nextTitleLevelFormatted = `${nextData.color.code}${nextData.bold || nextData.semi ? "§l" : ""}[${romanNumeral(
			index + 1 > (max ?? 5) ? 1 : index + 1
		)}]`;
...
export class MultiPVPDuelsGameMode {
	...
	public constructor(data: APIData, title: string, short: string, long: string) {
		...
		const { formatted, bold, semi, max, index, color, req, inc } = getTitle(this.overall.wins, title);
...
const assignTitles = (data: SinglePVPDuelsGameMode | SingleDuelsGameMode, title: string) => {
	const { formatted, bold, semi, max, index, color, req, inc } = getTitle(data.wins, title);

	data.titleFormatted = formatted;

	data.titleLevelFormatted = `${color.code}${bold || semi ? "§l" : ""}[${romanNumeral(index)}]`;

	const nextData = getTitle(req + index * inc, title);

	data.nextTitleLevelFormatted = `${nextData.color.code}${nextData.bold || nextData.semi ? "§l" : ""}[${romanNumeral(
		index + 1 > (max ?? 5) ? 1 : index + 1
	)}]`;
...
export class UHCDuels {
	...
	public constructor(data: APIData) {
		...
		const { formatted, bold, semi, max, index, color, req, inc } = getTitle(this.overall.wins, "UHC");
