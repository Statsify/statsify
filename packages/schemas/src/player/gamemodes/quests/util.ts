...
import { APIData, Constructor, UnwrapConstructor, removeFormatting } from "@statsify/util";
...
export interface CreateQuestsOptions<DailyFields extends string, WeeklyFields extends string> {
	game: FormattedGame;
	fieldPrefix?: string;
...
const processQuests = (instance: Record<string, number>, quests: APIData, time: QuestTime, options: QuestOption<string>[], fieldPrefix?: string) => {
...
export function createGameModeQuests<DailyFields extends string, WeeklyFields extends string>({
	game,
	fieldPrefix,
	daily,
	weekly,
}: CreateQuestsOptions<DailyFields, WeeklyFields>): GameWithQuests<DailyFields, WeeklyFields> {
	class Daily {
		[key: string]: number;
...
type IQuestInstance<Time extends QuestTime, GamesWithQuests extends BaseGamesWithQuestsRecord> = Constructor<{
	[K in keyof GamesWithQuests]: GamesWithQuests[K] extends GameWithQuests<string, string> ? UnwrapConstructor<GamesWithQuests[K][Time]> : never;
}>;
...
export function createQuestsInstance<Time extends QuestTime, GamesWithQuests extends BaseGamesWithQuestsRecord>(
	...
	const modes = Object.entries(record) as [keyof typeof FormattedGame, GameWithQuests<string, string>][];
