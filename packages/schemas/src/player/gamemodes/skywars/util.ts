...
export const getLevelProgress = (xp: number, level: number): Progression => {
  ...
  return new Progression(current, XP_TO_NEXT_LEVEL[TOTAL_XP.findIndex((x) => x > xp)]);
};
...
const SCHEME_MAP = {
  ...
  ender: createMultiDigitColorScheme(["§3", "§2", "§8", "§2", "§a", "§3"]),
  mite: createMultiDigitColorScheme(["§5", "§d", "§d", "§6", "§e", "§e"]),
