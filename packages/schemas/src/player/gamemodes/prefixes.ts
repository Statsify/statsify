 ...
 */
const getPrefixRequirement = <T extends unknown[] = []>(prefixes: Prefixes<T>, score: number, skip = 0): number => {
	const prefixIndex = prefixes.findIndex((requirement) => requirement.req > (score || 0));

	return prefixIndex === -1 ? prefixes.at(-1)!.req : prefixes[Math.min(prefixIndex + skip - 1, prefixes.length - 1)].req || 0;
};

export const createPrefixProgression = <T extends unknown[] = []>(prefixes: Prefixes<T>, score: number) => {
...
export const defaultPrefix = <T extends unknown[] = []>(prefixes: Prefixes<T>, options?: Omit<FormatPrefixOptions<T>, "prefixes" | "score">) =>
	getFormattedPrefix({ prefixes, score: prefixes[0].req, ...options });

const RAINBOW_COLORS = ["c", "6", "e", "a", "b", "d", "9"];

export const rainbow = (text: string) => [...text].map((l, i) => `§${RAINBOW_COLORS[i % RAINBOW_COLORS.length]}${l}`).join("");
