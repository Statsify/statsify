...
const tokens = new Map<Constructor, MetadataEntry[]>();

export function scanMetadata(target: Constructor) {
  if (tokens.has(target)) return tokens.get(target)!;

  const metadata = getMetadataEntries(target);

  tokens.set(target, metadata);
...
function getMetadataEntries(
  ...
  for (const [key, value] of entries) {
    ...
    const subMetadataEntries = getMetadataEntries(
      value.type.type,
      path,
      name,
    ).map(([keyPath, metadata]) => {
      if (!metadata.leaderboard.additionalFields?.length)
        metadata.leaderboard.additionalFields =
          value.leaderboard.additionalFields;
      ...
      if (!metadata.historical.additionalFields?.length)
        metadata.historical.additionalFields =
          value.historical.additionalFields;
...
if (import.meta.vitest) {
  ...
  const stringMetadata = (name: string): FieldMetadata => {
    ...
    const fieldName = prettify(
      name.slice(Math.max(0, lastIndexOfDot === -1 ? 0 : lastIndexOfDot + 1)),
    );
  ...
  suite("Schema Metadata", () => {
    it("should read and write basic string metadata", () => {
      ...
      expect(scanMetadata(Clazz)).toEqual<MetadataEntry[]>([
    ...
    it("should read and write with inherited metadata", () => {
      ...
      expect(scanMetadata(ChildClazz)).toEqual<MetadataEntry[]>([
        ["fieldA", stringMetadata("fieldA")],
    ...
    it("should carry down metadata", () => {
      ...
      class ParentClazz {
        @Field({
          leaderboard: { additionalFields: ["fieldA"], extraDisplay: "fieldA" },
        })
        public fieldA: ChildClazz;
      }

      const [[, { leaderboard }]] = scanMetadata(ParentClazz);
