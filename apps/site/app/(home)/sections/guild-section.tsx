    ...
    <BaseSection background="guilds" className="flex-col items-center relative overflow-hidden">
      ...
      <div className="mx-4 lg:mx-0 flex flex-col gap-4 max-w-120 xl:max-w-200 text-mc-white text-center lg:text-start">
        ...
        <p className="text-mc-1.75 lg:text-mc-2 leading-6 text-center">
          ...
          within the <Command command="/guild" /> command. Want to see your guild top GEXP but lazy to log on? No
          problem! Just run <Command command="/guild top" /> and view your top grinders. Use{" "}
          <Command command="/guild member" /> to view your player information inside your guild. And many more commands
          like <Command command="/guild leaderboard" /> and <Command command="/guild overall" />.
        </p>
      </div>
...
function GuildOverall({ guild, className }: { guild: Guild; className?: string }) {
  return (
    <div
      className={cn(
        "relative grid grid-cols-1 xl:grid-cols-2 gap-2 p-4 z-10 shadow-[8px_8px_0_rgb(0_0_0_/_0.5)]",
        className
      )}
    >
      <Box className="xl:col-span-2 text-center">
        <MinecraftText className="text-mc-3 md:text-mc-4">{guild.nameFormatted}</MinecraftText>
      </Box>
  ...
  return (
    <div className="relative grid grid-cols-content-1 lg:grid-cols-content-2 gap-2 p-4 shadow-[8px_8px_0_rgb(0_0_0_/_0.5)]">
      ...
      <Box className="content:flex content:justify-center content:items-center content:gap-4">
        <MinecraftText className="text-mc-3 md:text-mc-4">{member.displayName ?? ""}</MinecraftText>
