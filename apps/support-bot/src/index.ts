...
await Promise.all([I18nLoaderService, FontLoaderService, WinterThemeService, MongoLoaderService].map((service) => Container.get(service).init()));
...
await poster.post(commands, config("supportBot.applicationId"), config("supportBot.guild"));
