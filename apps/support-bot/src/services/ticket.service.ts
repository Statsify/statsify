...
export class TicketService {
	...
	public async init() {
		...
		tickets.forEach((ticket) => {
			listener.addHook(ticket.channel, (interaction) => this.close(interaction.getChannelId()!, "channel", interaction.getUserId()));
	...
	public async create(guildId: string, user: APIUser, username: string, issue: string) {
		const channel = await this.channelService.create(config("supportBot.guild"), {
			...
			permission_overwrites: [
				...
				},
				{
					...
					id: user.id,
					allow: `${
						PermissionFlagsBits.ViewChannel | PermissionFlagsBits.SendMessages | PermissionFlagsBits.AttachFiles | PermissionFlagsBits.EmbedLinks
					}`,
		...
		listener.addHook(closeTicketButton.getCustomId(), (interaction) => this.close(interaction.getChannelId()!, "channel", interaction.getUserId()));
	...
	public async close(channelIdOrOwnerId: string, type: "channel" | "owner", userId: string, reason = "N/A") {
		...
		messages.forEach((m) => {
			...
			const message = [`${m.author.username}#${m.author.discriminator}: ${m.content}`, ...m.attachments.map((a) => a.url)].join("\n");
		...
		const embed = new EmbedBuilder()
			...
			.color(STATUS_COLORS.info)
			.description(
				[`Owner: <@${ticket.owner}>`, `Closer: <@${userId}>`, `Participants: ${[...participants].join(", ")}`, `Reason: ${reason}`]
					.map((m) => `\`•\` ${m}`)
					.join("\n")
			);
	...
	private async copyUsername(interaction: Interaction) {
		const ticket = await this.ticketModel.findOne().where("channel").equals(interaction.getChannelId()).select({ username: true }).lean().exec();
