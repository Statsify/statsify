...
export class GuildMemberUpdateEventListener extends AbstractEventListener<GatewayDispatchEvents.GuildMemberUpdate> {
	...
	public async onEvent(data: GatewayGuildMemberUpdateDispatchData): Promise<void> {
		...
		if (currentRoleTier && currentRoleTier > (currentTier ?? 0)) return this.handlePremiumAdd(memberId, currentRoleTier);
	...
	private async handlePremiumAdd(memberId: string, tier: PremiumTier) {
		...
		const embed = new EmbedBuilder()
			...
			.description((t) => `We are very excited to have you as a ${tierName} member. Enjoy your perks! ${t("emojis:heart")}`);
	...
	private log(message: string) {
		...
		const embed = new EmbedBuilder().color(message.includes("ADDING") ? STATUS_COLORS.success : STATUS_COLORS.error).description(message);
