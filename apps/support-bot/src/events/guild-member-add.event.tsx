...
import { AbstractEventListener, ApiService, EmbedBuilder, GuildService, IMessage, MemberService, MessageService } from "@statsify/discord";
...
export class GuildMemberAddEventListener extends AbstractEventListener<GatewayDispatchEvents.GuildMemberAdd> {
	...
	public async onEvent(data: GatewayGuildMemberAddDispatchData): Promise<void> {
		...
		const [avatar, background] = await Promise.all([this.getDiscordAvatar(data), getBackground("minecraft", "overall")]);

		const username = data.user!.username;

		const canvas = render(
			<WelcomeProfile memberCount={guild.approximate_member_count ?? 0} avatar={avatar} username={username} background={background} />
		);
