...
import { ApiService, Command, CommandContext, EmbedBuilder, ErrorMessage, PaginateService, TextArgument } from "@statsify/discord";
...
export class CommandsCommand {
	...
	public async run(context: CommandContext) {
		...
		return this.paginateService.scrollingPagination(
			context,
			groups.map(
				(group, index) => () => this.createTopPage(index, group, totalCommands, context.option<string>("highlight")?.toLowerCase().replace(" ", "_"))
			)
		);
	}

	private createTopPage(page: number, commands: [commandName: string, usage: number][], totalCommands: number, highlight: string) {
			...
			.description((t) =>
				commands
					.map(([command, usage], index) => {
						...
						const commandName = `${command.includes(highlight) ? "**" : ""}${prettify(command)}${command.includes(highlight) ? "**" : ""}`;
						const percentage = Math.round((usage / totalCommands) * 100);

						return `\`#${String(position).padStart(String(page * COMMANDS_PER_PAGE + COMMANDS_PER_PAGE).length, "0")}\` ${commandName}: **${t(
							usage
						)}** \`(${t(percentage)}%)\``;
					})
					.join("\n")
