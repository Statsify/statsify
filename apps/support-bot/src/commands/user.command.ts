...
export class UserCommand {
	...
	public async run(context: CommandContext) {
		...
		this.addField(embed, "Footer Icon", user.footer?.icon, (v) => `\`${prettify(logos.find(([, l]) => l === v)?.[0] ?? "DEFAULT")}\``);
	...
	private addField<T>(embed: EmbedBuilder, title: string, value: T, fmt: (value: T extends undefined ? never : T) => string = (v) => `\`${v}\``) {
