...
const runLimit = async (constructors, prefixes) => {
	constructors.forEach(async (constructor, i) => {
		...
		fields.forEach(([key, value]) => {
			...
			if (!value.leaderboard.enabled || (prefixes ? !value.historical.enabled : false)) oldLeaderboardPipeline.del(path);
		});

		await oldLeaderboardPipeline.exec();

		const leaderboards = prefixes ? fields.filter(([, value]) => value.historical.enabled) : fields.filter(([, value]) => value.leaderboard.enabled);
		...
		logger.log(
			`There are ${memberCount.toLocaleString()} possible members in the ${prefixes ? prefixes[i].toLowerCase() : "lifetime"} ${name} leaderboards`
		);
	});
};
...
const limit = async () => {
	await Promise.all([
		...
		runLimit([Player, Player, Player], [CurrentHistoricalType.DAILY, CurrentHistoricalType.WEEKLY, CurrentHistoricalType.MONTHLY]),
	]);
};
