...
type Difficulty = keyof Bingo;
type Category = keyof Bingo[Difficulty];

type Board<D extends Difficulty, C extends Category> = {
...
type Task<D extends Difficulty = Difficulty, C extends Category = Category> = {
  game: string;
  name: string;
  description: string;
  progress: number;
...
const boards: { [D in Difficulty]: { [C in Category]: Board<D, C> } } = {
  easy: {
    casual: {
      blackoutReward: {
        description: [
      ...
      diagonalRewards: [
        {
          description: "§8+§610,000 §7Arcade Coins§r",
          name: "Diagonal One",
        },
        {
          description: "§8+§25,000 §7Murder Mystery Tokens§r",
  ...
  hard: {
    casual: {
      ...
      diagonalRewards: [
        {
          description: "§8+§650,000 §7Arcade Coins§r",
          name: "Diagonal One",
        },
        {
          description: "§8+§225,000 §7Murder Mystery Tokens§r",
...
function BingoBoard({ category, difficulty }: { category: Category; difficulty: Difficulty }) {
  ...
  return (
    <>
      <div className="grid grid-cols-6 grid-rows-6 gap-2 **:text-mc-1.5">
        ...
        <div className="row-start-2 col-start-2 grid grid-cols-subgrid grid-rows-subgrid row-span-4 col-span-4">
          {bingo.tasks.map((task) => <TaskCard key={task.field} task={task} />)}
        </div>
...
function RewardCard({
  ...
  return (
    <Box {...props} containerClass={`@container ${containerClass}`} contentClass={`flex flex-col gap-2 ${contentClass}`}>
      <p className={cn("font-bold text-mc-pink text-center leading-[16px]", variant === "blackout" && "text-mc-dark-purple")}>{reward.name} Reward</p>
...
function TaskCard({ task }: { task: Task }) {
  return (
    <Box containerClass="@container" contentClass="flex flex-col gap-2 text-center">
      <div>
