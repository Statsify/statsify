...
export function PlayerSection({ players }: { players: [Player, Player, Player, Player] }) {
  ...
  return (
    <BaseSection background={games[gameIndex].tab} className="flex-col lg:flex-row justify-around lg:items-center lg:py-0">
        ...
        <div className="w-fit flex flex-col gap-4">
          ...
          <div className="grid grid-areas-stack">
            {games.map(({ tab, component: GamePreview }, index) => (
              ...
              >
                <GamePreview
                  className={cn("grid-area-stack invisible", index === gameIndex && "visible")}
                />
              </PlayerProvider>
            ))}
          </div>
