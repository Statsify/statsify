...
export function GuildSection({ guild }: { guild: Guild }) {
  return (
    <BaseSection background="guild" className="flex-col items-center relative py-24">
      ...
      <div className="p-4 bg-white/10 backdrop-blur-[128px] grid grid-cols-2 gap-2">
        ...
        <Box containerClass="col-span-2 text-center"><div className="flex gap-2 items-center justify-center">
          {guild.preferredGames.map(game => <span>{game.substring(0,3)}</span>)}</div>
        </Box>
      </div>

      <div className="p-4 bg-white/10 backdrop-blur-[128px] grid grid-cols-3 gap-2">
      ...
      <div className="p-4 bg-white/10 backdrop-blur-[128px]">
        <div className="flex gap-2">
      ...
      <div className="grid grid-cols-[repeat(5,minmax(min-content,max-content))] gap-2">
        {guild.members.map((member) => (
          <Box key={member.uuid}>
            <MinecraftText>{member.displayName ?? "name"}</MinecraftText>
          </Box>
        ))}
      </div>
