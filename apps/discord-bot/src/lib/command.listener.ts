...
import { AbstractCommandListener, ApiService, CommandContext, CommandResolvable, ErrorMessage, IMessage, Interaction } from "@statsify/discord";
...
export class CommandListener extends AbstractCommandListener {
	...
	private constructor(client: WebsocketShard | InteractionServer, rest: RestClient, commands: Map<string, CommandResolvable>) {
		super(client as InteractionServer, rest, commands, config("discordBot.applicationId"), config("discordBot.port", { required: false })!);
	...
	protected async onCommand(interaction: Interaction): Promise<void> {
		...
		const preconditions = [this.tierPrecondition.bind(this, command, user), this.cooldownPrecondition.bind(this, parentCommand, user, id)];
	...
	public static create(client: WebsocketShard | InteractionServer, rest: RestClient, commands: Map<string, CommandResolvable>) {
