...
const getDefaultApiMode = <T extends string>(modes: GameModes<T>) => modes.getApiModes()[0];
...
export const mapBackground = <T extends GamesWithBackgrounds>(modes: GameModes<T>, mode: T[number]): [game: string, mode: string] => {
	switch (modes) {
		case BEDWARS_MODES: {
			let map: string;
...
const HYPIXEL_GAME_LIST = Object.fromEntries(Object.entries(GAME_ID_TO_MODES).map(([key, g]) => [key, g?.getHypixelModes()]));
