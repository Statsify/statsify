...
export class GuildTopSubCommand extends GuildLeaderboardSubCommand {
	...
	public async top(context: CommandContext) {
		...
		const changePage = (fn: (interaction: Interaction) => Partial<GuildTopPageState>) => async (interaction: Interaction) => {
			...
			const message = await this.getGuildTopPageMessage(cache, components, props, mode[1], mode[0], state.page ?? page, theme);
		...
		const message = await this.getGuildTopPageMessage(cache, components, props, modes[modeIndex][1], modes[modeIndex][0], page, theme);
	...
	private async generateGuildTopPage(props: BaseGuildTopProfileProps, title: string, key: GuildTopKey, page: number, theme?: Theme) {
