...
export const GuildMemberProfile = ({ player, guild, skin, badge, logo, background, user, t }: GuildMemberProfileProps) => {
	...
	const positions = guild.expHistory.slice(0, 7).map((_, i) => {
		const position = guild.members.sort((a, b) => Number(b.expHistory[i]) - Number(a.expHistory[i])).findIndex((m) => m.uuid === member.uuid);
