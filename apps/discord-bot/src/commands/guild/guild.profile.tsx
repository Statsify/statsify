...
export const GuildProfile = ({ guild, background, logo, user, t, guildMaster, skin, ranking, gameIcons, page }: GuildProfileProps) => {
	...
	switch (page) {
		...
		default:
		case "overall":
			pageEl = <GuildOverallPage guild={guild} t={t} guildMaster={guildMaster} skin={skin} ranking={ranking} gameIcons={gameIcons} />;
			break;
	}
	...
	return (
		<>
			...
			<GuildBlock title={t("stats.guild.averageMemberGexp")} width="100%">
				<Table.table>
					<Table.tr>
						<Table.td title={t("stats.guild.daily")} value={t(ratio(guild.daily, guild.members.length))} color="§2" size="small" />
						<Table.td title={t("stats.guild.weekly")} value={t(ratio(guild.weekly, guild.members.length))} color="§2" size="small" />
						<Table.td title={t("stats.guild.monthly")} value={t(ratio(guild.monthly, guild.members.length))} color="§2" size="small" />
					</Table.tr>
				</Table.table>
			</GuildBlock>
			<GuildBlock title="GEXP History" width="100%">
				<GexpTable dates={guild.expHistoryDays} expHistory={guild.expHistory} scaledExpHistory={guild.scaledExpHistory} t={t} />
			</GuildBlock>
		</>
	);
};
...
const formatGuildAchievement = (achievement: Progression, t: LocalizeFunction) => `${t(achievement.current)}§7/${t(achievement.max!)}`;
		...
		<>
			<GuildBlock width="100%" title="Guild Achievements">
				<Table.table>
					...
					<Table.tr>
						<Table.td title={`Winners ${achievements.winnersTier}`} value={formatGuildAchievement(achievements.winnersProgression, t)} color="§e" />
						<Table.td title={`Family ${achievements.familyTier}`} value={formatGuildAchievement(achievements.familyProgression, t)} color="§e" />
					</Table.tr>
				</Table.table>
			</GuildBlock>
