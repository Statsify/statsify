...
export const HistoricalGeneralProfile = ({ background, logo, player, skin, t, time, badge, user }: BaseProfileProps) => {
	const { general, quests, challenges } = player.stats;

	return (
		<Container background={background}>
			<Header name={player.prefixName} skin={skin} title={`§l${FormattedGame.GENERAL} Stats`} time={time} badge={badge} />
			<Table.table>
				<Table.tr>
					...
					<Table.td title={t("stats.event-exp")} value={t(general.events.christmas2022.exp)} color="§c" />
				</Table.tr>
				<Table.tr>
