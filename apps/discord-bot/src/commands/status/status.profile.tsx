	...
	<Table.table>
		<Table.table>
			...
			<Table.tr>
				...
				<If condition={status.mode}>{(mode) => <Table.td title={t("stats.mode")} value={mapGame(status.game.id, mode)} color="§e" />}</If>
...
export const StatusProfile = ({ status, skin, badge, background, logo, user, t }: StatusProfileProps) => {
