...
import { ApiService, Command, CommandContext, IMessage, PlayerArgument } from "@statsify/discord";
...
export class StatusCommand {
	...
	public async run(context: CommandContext): Promise<IMessage> {
		...
		const canvas = render(
			<StatusProfile status={status} skin={skin} logo={logo} badge={badge} background={background} t={t} user={user} />,
			getTheme(user)
		);
