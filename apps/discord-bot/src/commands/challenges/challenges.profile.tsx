...
import { ChallengeModes, Challenges, FormattedGame, GameChallenges, GameId, GameMode, MetadataScanner } from "@statsify/schemas";
...
export const ChallengesProfile = ({ skin, player, background, logo, user, badge, mode, t, time, gameIcons }: ChallengeProfileProps) => {
	...
	switch (api) {
		...
		default:
			table = <GameTable gameChallenges={challenges[api]} constructor={challenges[api].constructor} t={t} />;
			break;
	}
	...
	return (
		<Container background={background}>
			<Header skin={skin} name={player.prefixName} badge={badge} title={`§l§aChallenges §r(${title}§r)`} sidebar={sidebar} time={time} />
			{table}
