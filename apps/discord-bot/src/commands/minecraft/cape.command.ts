...
import { Command, CommandContext, ErrorMessage, MojangPlayerArgument, PaginateService } from "@statsify/discord";
...
export class CapeCommand {
	...
	private async getMojangCape(player: AshconResponse) {
		const image = player.textures.cape?.url ? await loadImage(player.textures.cape.url).catch(() => null) : null;
