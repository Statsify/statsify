...
export class ColorsCommand {
	public async run() {
		const embed = new EmbedBuilder()
			...
			.description((t) => {
				let desc = `**${t("embeds.colors.color")} • ${t("embeds.colors.mainHex")} • ${t("embeds.colors.shadowHex")}**\n`;

				minecraftColors.forEach((color) => {
					desc += `${t(`emojis:colors.${[color.code.slice(1)]}`)} \`${color.code}\` • \`${color.hex}\` • \`${mcShadow(color.hex)}\`\n`;
				});
