...
import { ApiService, Command, CommandContext, ErrorMessage, FileArgument, IMessage, LocalizeFunction, SubCommand } from "@statsify/discord";
...
export class BadgeCommand {
	...
	private async run(context: CommandContext, mode: "view" | "set" | "reset"): Promise<IMessage> {
		...
		switch (mode) {
			...
			case "set": {
				...
				ctx.drawImage(badge, 0, 0, badge.width, badge.height, (canvas.width - width) / 2, (canvas.height - height) / 2, width, height);
	...
	private async getProfile(t: LocalizeFunction, user: User, badge?: Image | Canvas) {
		...
		const canvas = render(
			<DemoProfile background={background} logo={logo} player={player} skin={skin} badge={badge} user={user} message={t("config.badge.profile")} />,
			getTheme(user)
		);
