...
const NormalTable = ({ quests, t, gameIcons, colorPalette, time }: NormalTableProps) => {
	...
	if (time === QuestTime.Overall) {
		const entries: GameEntry[] = questEntries.sort((a, b) => (b[1]?.total ?? 0) - (a[1]?.total ?? 0)).map(([k, v]) => [k as GameId, t(v.total)]);
	...
	const entries: GameEntry[] = questEntries
		...
		.map(([k, v, total]) => {
			...
			if (completed === total) {
				textColor = "§a";
				boxColor = useGradient("horizontal", [GRADIENT_OFFSET, BOX_COLOR], [1, "hsla(145, 45%, 44%, 0.5)"]);
			} else if (completed >= 1) {
				textColor = "§6";
				boxColor = useGradient("horizontal", [GRADIENT_OFFSET, BOX_COLOR], [1, "hsla(42, 17%, 48%, 0.5)"]);
			} else {
				textColor = "§c";
				boxColor = useGradient("horizontal", [GRADIENT_OFFSET, BOX_COLOR], [1, "hsla(337, 31%, 43%, 0.5)"]);
			}
		...
		.map(([quest, completions]) => {
			...
			return (
				<box width="100%">
					...
					<div width="remaining" />
					{isOverall ? <text>{t(completions)}</text> : <img margin={2} image={logos[completions]} />}
				</box>
			);
		});
...
export const QuestsProfile = ({ skin, player, background, logo, user, badge, mode, t, gameIcons, time, logos }: QuestProfileProps) => {
	...
	switch (api) {
		case "overall":
			table = <NormalTable quests={quests[period]} time={time} t={t} gameIcons={gameIcons} colorPalette={colorPalette} />;
			break;
	...
	if (time === QuestTime.Weekly) {
		...
		(historicalTime as HistoricalTimeData).startTime = dt.plus({ days: 4 }).toMillis() < Date.now() ? dt.plus({ days: 4 }) : dt.minus({ days: 3 });

		(historicalTime as HistoricalTimeData).endTime = (historicalTime as HistoricalTimeData).startTime!.plus({ days: 6 });
	}

	return (
		<Container background={background}>
			<Header skin={skin} name={player.prefixName} badge={badge} title={`§l§eQuests §r(${title}§r)`} sidebar={sidebar} time={historicalTime} />
			{table}
