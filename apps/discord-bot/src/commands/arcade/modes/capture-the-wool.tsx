...
export const CaptureTheWoolTable = ({ stats, t, time }: CaptureTheWoolTableProps) => (
	<Table.table>
		<Table.ts title="§6Overall">
			...
			<Table.tr>
				<Table.td title={t("stats.woolPickedUp")} value={t(stats.woolPickedUp)} color="§2" />
				<Table.td title={t("stats.woolCaptured")} value={t(stats.woolCaptured)} color="§b" />
			</Table.tr>
			<Historical.exclude time={time}>
				<Table.tr>
					<If condition={stats.fastestWin > 0}>
						<Table.td title={t("stats.fastestWin")} value={formatTime(stats.fastestWin)} color="§e" />
					</If>
					<If condition={stats.fastestWoolCapture > 0}>
						<Table.td title={t("stats.fastestWoolCapture")} value={formatTime(stats.fastestWoolCapture)} color="§6" />
					</If>
					<If condition={stats.longestGame > 0}>
						<Table.td title={t("stats.longestGame")} value={formatTime(stats.longestGame)} color="§d" />
					</If>
				</Table.tr>
			</Historical.exclude>
		</Table.ts>
