...
export class PlayerLeaderboardCommand extends BaseLeaderboardCommand {
	...
	private async run<T extends GamesWithBackgrounds>(context: CommandContext, prefix: keyof PlayerStats, modes: GameModes<T>) {
		const leaderboard = context.option<string>("leaderboard");

		const field = `stats.${prefix}.${leaderboard.replaceAll(" ", ".")}`;

		const background = await getBackground(...mapBackground(modes, modes.getModes()[0].api));
