...
type GetLeaderboard = (field: string, input: string | number, type: LeaderboardQuery) => Promise<PostLeaderboardResponse | null>;
...
export class BaseLeaderboardCommand {
	protected async createLeaderboard({ context, background, field, getLeaderboard, type, getLeaderboardDataIcon }: CreateLeaderboardOptions) {
		...
		const changePage = (fn: () => LeaderboardParams) => async (interaction: Interaction) => {
			...
			const [message, page] = await this.getLeaderboardMessage(user, cache, type, getLeaderboard, field, params, props, getLeaderboardDataIcon);
	...
	private async getLeaderboardMessage(
		...
		const [message, page] = await this.renderLeaderboardMessage(user, type, getLeaderboard, field, params, props, getLeaderboardDataIcon);
