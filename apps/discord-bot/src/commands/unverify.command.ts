...
import { ApiService, Command, CommandContext, EmbedBuilder, ErrorMessage, IMessage, MemberService } from "@statsify/discord";
...
export class UnverifyCommand {
	...
	public async run(context: CommandContext): Promise<IMessage> {
		...
		const embed = new EmbedBuilder().description((t) => t("verification.successfulUnverification")).color(STATUS_COLORS.success);
