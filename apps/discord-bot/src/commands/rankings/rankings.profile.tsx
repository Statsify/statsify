...
export const RankingsProfile = ({ background, logo, user, data, t, player, skin, game, badge }: RankingsProfileProps) => {
	const listTitles = ["Statistic", "Pos", "Value"];
	if (!game) listTitles.unshift("Game");

	const titles = listTitles.map((field) => (
		<box width={field === "Statistic" ? "remaining" : "100%"} border={{ topLeft: 4, topRight: 4, bottomLeft: 0, bottomRight: 0 }}>
			<text>§l{field}</text>
		</box>
	));
	...
	return (
		<Container background={background}>
			<Header name={player.prefixName} skin={skin} time="LIVE" title={`§l§bLeaderboard Positions §r(§l${formattedGame}§r)`} badge={badge} />
