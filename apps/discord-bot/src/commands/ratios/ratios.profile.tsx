...
const Estimation = ({ ratioTitle, numeratorTitle, numeratorValue, denominatorValue, t }: RatioProps) => {
	...
	const newNumerator = Math.ceil((1 + targetRatio / (targetRatio + 1) - ratioValue / targetRatio) * numeratorValue + denominatorValue);
...
export type RatioWithStats = [numeratorValue: number, denominatorValue: number, numeratorTitle: string, ratioTitle: string];
...
export const RatiosProfile = ({ skin, player, background, logo, user, badge, mode, ratios, t, gameName }: RatiosProfileProps) => (
	<Container background={background}>
		<Header badge={badge} time="LIVE" name={player.prefixName} skin={skin} title={`§l${gameName} §fRatios §r(${mode.formatted})`} />
		<Table.table>
			<Table.tr>
