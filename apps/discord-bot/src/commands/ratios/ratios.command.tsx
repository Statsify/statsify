...
import { ApiService, Command, CommandContext, Page, PaginateService, PlayerArgument, SubCommand } from "@statsify/discord";
...
export class RatiosCommand {
	...
	public blitzsg(context: CommandContext) {
		return this.run(context, BLITZSG_MODES, (player, modes) => {
			...
			const filteredKits = kits.sort((a, b) => (blitzsg[b.api] as BlitzSGKit).exp - (blitzsg[a.api] as BlitzSGKit).exp).slice(0, 24);
		...
		const pages: Page[] = displayedModes.map((mode, index) => ({
			...
			generator: async (t) => {
				...
				const props: RatiosProfileProps = {
					...
					ratios: ratios.map((r) => [stats[r[0] as keyof typeof stats], stats[r[1] as keyof typeof stats], prettify(r[0]), r[3]]),
				};
