...
import { GuildNotFoundException, GuildQuery, HypixelCache, PlayerNotFoundException } from "@statsify/api-client";
...
export class GuildService {
	...
	public async get(inputtedTag: string, type: GuildQuery, cache: HypixelCache): Promise<Guild | null> {
		...
		const fetchMembers = guild.members.map(async (member) => {
			...
			const combinedExpHistory: Record<string, number> = {
				...cacheMember?.expHistoryDays?.reduce((acc, day, index) => ({ ...acc, [day]: cacheMember.expHistory[index] }), {}),
		...
		await this.playerModel.updateMany({ guildId: guild.id }).where("uuid").in(requireGuildId).lean().exec();
	...
	private async getCachedGuild(tag: string, type: GuildQuery): Promise<[guild: Guild | null, tag: string, displayName?: string]> {
		tag = tag.toLowerCase();
