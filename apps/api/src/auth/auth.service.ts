...
export class AuthService {
	...
	public async limited(apiKey: string, weight: number, role: AuthRole) {
		...
		const weightedTotal = (requests[1] as string[]).filter((_, i) => i % 2 === 0).reduce((acc, key) => acc + Number(key.split(":")[1]), 0);

		const resetTime = 60_000 - (time - (requests[1] as [Error | null, number])[1]);

		if (weightedTotal > apiKeyLimit) {
			this.logger.warn(`${name} has exceeded their request limit of ${apiKeyLimit} and has requested ${weightedTotal} times`);
	...
	public async getKey(apiKey: string): Promise<Key> {
		...
		const recentRequests = (requests[1] as string[]).filter((_, i) => i % 2 === 0).reduce((acc, key) => acc + Number(key.split(":")[1]), 0);
