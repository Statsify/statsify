...
import { HypixelCache, PlayerNotFoundException, RecentGamesNotFoundException, StatusNotFoundException } from "@statsify/api-client";
...
export class PlayerService {
	 ...
	 */
	public async get(tag: string, cacheLevel: HypixelCache, selector?: Record<string, boolean>): Promise<Player | null> {
		...
		if (player) {
			...
			if (mongoPlayer && mongoPlayer.username !== player.username) await this.playerSearchService.delete(mongoPlayer.username);
	...
	private async findMongoDocument(tag: string, selector: Record<string, boolean> = {}): Promise<Flatten<Player> | null> {
