...
export class CommandsService {
	...
	public async incrementCommandRun(command: string) {
		...
		const incrementedCommands = Object.fromEntries(commands.map((_, index) => [`usage.${commands.slice(0, index + 1).join("_")}`, 1]));

		incrementedCommands["usage.commands"] = 1;

		await this.commandsModel.updateOne({ name: "commands" }, { $inc: incrementedCommands }, { upsert: true }).lean().exec();
	}
